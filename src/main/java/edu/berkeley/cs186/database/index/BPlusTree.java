...
public class BPlusTree {
    ...
    public void put(DataBox key, RecordId rid) {
        ...
        // CASE 1: NO OVERFLOW IN ROOT
        if (result.equals(Optional.empty())) {
            return;
        }

        // CASE 2: OVERFLOW IN ROOT
        else {

            DataBox splitKey = result.get().getFirst();
            Long right_node_page_num = result.get().getSecond();
            ...
            updateRoot((BPlusNode) newNode);

            /**
             * 
             * List<DataBox> keys = ((InnerNode) root).getKeys();
             * List<Long> children = ((InnerNode) root).getChildren();
             * 
             * int j;
             * for (j = 0; j < keys.size(); ++j) {
             * if (splitKey.compareTo(keys.get(j)) < 0) {
             * break;
             * }
             * }
             * 
             * keys.add(j, splitKey);
             * children.add(j + 1, right_node_page_num);
             * 
             * // CASE 1: NO ROOT OVERFLOW
             * if (keys.size() <= 2 * order) {
             * return;
             * }
             * 
             * // CASE 2: ROOT OVERFLOW
             * else {
             * InnerNode newNode = new InnerNode(metadata, bufferManager, keys.subList(order
             * + 1, keys.size()),
             * children.subList(order + 1, children.size()), lockContext);
             * 
             ...
             * newChildren.add(newNode.getPage().getPageNum());
             * 
             * InnerNode parent = new InnerNode(metadata, bufferManager, keys.subList(order,
             * order + 1), newChildren,
             * lockContext);
             * updateRoot((BPlusNode) parent);
             * }
             **/
    ...
    public void bulkLoad(Iterator<Pair<DataBox, RecordId>> data, float fillFactor) {
        ...
        /**
         * Iterator<RecordId> check = this.scanAll();
         * int count = 0;
         * while (check.hasNext()) {
         * count++;
         * }
         * if (count != 0) {
         * throw new ("Tree is not empty");
         * }
         */
        ...
        while (data.hasNext()) {
            ...
            // CASE 1: ROOT DOES NOT OVERFLOW
            if (result.equals(Optional.empty())) {
                return;
            }

            // CASE 2: ROOT OVERFLOWS
            else {
                DataBox splitKey = result.get().getFirst();
                Long right_node_page_num = result.get().getSecond();
    ...
    private class BPlusTreeIterator implements Iterator<RecordId> {
        ...
        private BPlusTreeIterator(DataBox key) {
            ...
            index = current.getKeys().indexOf(key);
        }
        ...
        public boolean hasNext() {
            ...
            if (index < current.getKeys().size()) {
                return true;
            } else if (index == current.getKeys().size()) {
                Optional<LeafNode> potential = current.getRightSibling();
                index = 0;
                if (potential == null || potential.equals(Optional.empty())) {
                    return false;
                } else {
