...
class InnerNode extends BPlusNode {
    ...
    public Optional<Pair<DataBox, Long>> put(DataBox key, RecordId rid) {
        ...
        Optional<Pair<DataBox, Long>> result = this.getChild(i).put(key, rid);

        // CASE 1: NO OVERFLOW IN CHILD
        if (result.equals(Optional.empty())) {
            this.sync();
            return Optional.empty();
        }

        // CASE 2: OVERFLOW IN CHILD
        else {
            ...
            // CASE 1: NO OVERFLOW IN SELF
            if (keys.size() <= 2 * order) {
                this.sync();
                return Optional.empty();
            }

            // CASE 2: OVERFLOW IN SELF
            else {
                InnerNode newNode = new InnerNode(metadata, bufferManager, keys.subList(order + 1, keys.size()),
                        children.subList(order + 1, children.size()), treeContext);

                Pair<DataBox, Long> p = new Pair<DataBox, Long>(keys.get(order), newNode.getPage().getPageNum());
    ...
    public Optional<Pair<DataBox, Long>> bulkLoad(Iterator<Pair<DataBox, RecordId>> data,
        ...
        // WHILE NODE IS NOT FULL
        while (keys.size() <= 2 * order) {
            Optional<Pair<DataBox, Long>> result = this.getChild(children.size() - 1).bulkLoad(data, fillFactor);

            // CASE 1: CHILD DOES NOT HIT OCCUPANCY MAX
            if (result.equals(Optional.empty())) {
                this.sync();
                return Optional.empty();
            }

            // CASE 2: CHILD HITS OCCUPANCY MAX - ADD TO INNER NODE
            else {
                DataBox splitKey = result.get().getFirst();
                Long right_node_page_num = result.get().getSecond();
                keys.add(splitKey);
